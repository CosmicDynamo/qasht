    ...
    return {
        ...
        execTest: function (test) {
            ...
            if (lang.isFunction(test.setUp)) {
                try {
                    setup = test.setUp.defer(test, arguments);
                } catch (ex) {
                    return onError(ex);
                }
            }
            ...
            var run = when(setup, function () {
                return test.exec.defer(test, args);
            }, onError);

            function clean() {
                var cleaned = null;
                if (lang.isFunction(test.tearDown)) {
                    cleaned = test.tearDown.defer(test, args);
                }
            ...
            if (test.timeout) {
                this.timeout.next(this, [test], test.timeout || 1000);
            }
